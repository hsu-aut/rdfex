...
import rdfex.DataMap.ContainerVariableCountComparator;
...
public class MappingEngine {
	...
	public List<DataMap> getAllMappingDefinitions(Model model) {
		String queryString = "PREFIX rdfex: <http://www.w3id.org/hsu-aut/rdfex#>"
				+ "SELECT ?mappingDefinition ?source ?sourceType ?targetFormat ?queryLanguage ?query ?container ?snippet WHERE {"
				+ "?mappingDefinition rdfex:ontologicalSource ?ontologicalSource." 
				+ "?ontologicalSource rdfex:source ?source;" 
				+ "rdfex:sourceType ?sourceType;"
				+ "rdfex:queryLanguage ?queryLanguage;" 
				+ "rdfex:query ?query." 
				+ "?mappingDefinition rdfex:container ?container;" 
				+ "rdfex:targetFormat ?targetFormat;"
				+ "rdfex:snippet ?snippet." + "}";
